...
function add_neighbor_data(iface, property) {
    for (var i = 0; i < property.length; i++) {
        ...
        if (property[i]["Operation"] == "delete") {
            ...
            button_off= table_add_button(neighbors_table_id, neighbor_row, 'LED off', 'Resources', '', "led_off(this);");
            button_off.setAttribute("IPv6_address", value);

            button_on = table_add_button(neighbors_table_id, neighbor_row, 'LED on', 'Resources', '', "led_on(this);");
            button_on.setAttribute("IPv6_address", value);
...
function onAddClick() {
    ...
    button_off = table_add_button(neighbors_table_id, neighbor_row, 'LED off', 'Resources', '', "led_off(this);");
    button_off.setAttribute("IPv6_address", '2001:DB8::1');

    button_on = table_add_button(neighbors_table_id, neighbor_row, 'LED on', 'Resources', '', "led_on(this);");
    button_on.setAttribute("IPv6_address", '2001:DB8::1');

    neighbor_row.setAttribute("IPv6_address", '2001:DB8::1');

    table_add_data(neighbors_table_id, neighbor_row, '2001:DB8::1', 'IP address', '');
}

function onDelClick() {
    delete_neighbor('2001:DB8::1');
}
