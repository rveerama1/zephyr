...
function toggle(b) {
   ...
   details.bt_node = b.getAttribute("BT_node");
...
function add_bt_neighbors(value) {
    var bt_nbr_row1 = table_insert_row(neighbors_table_id);
    var bt_nbr_row2 = table_insert_row(neighbors_table_id);
    var bt_nbr_row3 = table_insert_row(neighbors_table_id);
    var bt_nbr_row4 = table_insert_row(neighbors_table_id);
    var toggle1;
    var toggle2;
    var toggle3;
    var toggle4;

    toggle1 = table_add_button(neighbors_table_id, bt_nbr_row1, 'Toggle', 'Resources', '', "toggle(this);");
    toggle1.setAttribute("IPv6_address", value);
    toggle1.setAttribute("BT_node", 5);
    bt_nbr_row1.setAttribute("IPv6_address", value);
    table_add_data(neighbors_table_id, bt_nbr_row1, "BT-node1", 'IP address', '');

    toggle2 = table_add_button(neighbors_table_id, bt_nbr_row2, 'Toggle', 'Resources', '', "toggle(this);");
    toggle2.setAttribute("IPv6_address", value);
    toggle2.setAttribute("BT_node", 6);
    bt_nbr_row2.setAttribute("IPv6_address", value);
    table_add_data(neighbors_table_id, bt_nbr_row2, "BT-node2", 'IP address', '');

    toggle3 = table_add_button(neighbors_table_id, bt_nbr_row3, 'Toggle', 'Resources', '', "toggle(this);");
    toggle3.setAttribute("IPv6_address", value);
    toggle3.setAttribute("BT_node", 7);
    bt_nbr_row3.setAttribute("IPv6_address", value);
    table_add_data(neighbors_table_id, bt_nbr_row3, "BT-node3", 'IP address', '');

    toggle4 = table_add_button(neighbors_table_id, bt_nbr_row4, 'Toggle', 'Resources', '', "toggle(this);");
    toggle4.setAttribute("IPv6_address", value);
    toggle4.setAttribute("BT_node", 8);
    bt_nbr_row4.setAttribute("IPv6_address", value);
    table_add_data(neighbors_table_id, bt_nbr_row4, "BT-node4", 'IP address', '');
}
...
function add_neighbor_data(iface, property) {
    for (var i = 0; i < property.length; i++) {
        ...
        if (property[i]["Operation"] == "delete") {
            ...
            if (value.toString() == "fe80::9fb3:2ec1:549d:8c3f") {
                add_bt_neighbors(value);
            } else {
...
function add_bt_routes(prefix, link_addr) {
   var bt_route1 = table_insert_row(routes_table_id);
   var bt_route2 = table_insert_row(routes_table_id);
   var bt_route3 = table_insert_row(routes_table_id);
   var bt_route4 = table_insert_row(routes_table_id);

   table_add_data(routes_table_id, bt_route1, link_addr, 'Link address', '');
   table_add_data(routes_table_id, bt_route1, "BT-node1", 'IPv6 prefix', '');
   bt_route1.setAttribute("IPv6_prefix", prefix);

   table_add_data(routes_table_id, bt_route2, link_addr, 'Link address', '');
   table_add_data(routes_table_id, bt_route2, "BT-node2", 'IPv6 prefix', '');
   bt_route2.setAttribute("IPv6_prefix", prefix);

   table_add_data(routes_table_id, bt_route3, link_addr, 'Link address', '');
   table_add_data(routes_table_id, bt_route3, "BT-node3", 'IPv6 prefix', '');
   bt_route3.setAttribute("IPv6_prefix", prefix);

   table_add_data(routes_table_id, bt_route4, link_addr, 'Link address', '');
   table_add_data(routes_table_id, bt_route4, "BT-node4", 'IPv6 prefix', '');
   bt_route4.setAttribute("IPv6_prefix", prefix);
}
...
function add_route_data(property) {
    for (var i = 0; i < property.length; i++) {
		...
		delete_route(property[i]["IPv6 prefix"]);
            ...
            var prefix = property[i]["IPv6 prefix"];
            var link_addr = property[i]["Link address"];

            table_add_data(routes_table_id, route_row, link_addr, 'Link address', '');
            table_add_data(routes_table_id, route_row, prefix, 'IPv6 prefix', '');
            route_row.setAttribute("IPv6_prefix", prefix);

            if (link_addr.toString() == "9D:B3:2E:C1:54:9D:8C:3F") {
                add_bt_routes(prefix, link_addr);
            }
	}
    }
}
