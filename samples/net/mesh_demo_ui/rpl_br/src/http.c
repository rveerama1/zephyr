...
static const struct json_obj_descr command_descr[] = {
	...
	JSON_OBJ_DESCR_PRIM(struct coap_command, bt_node, JSON_TOK_STRING),
};
				...
				void *user_data)
{
	...
	src = 1;
	dst = atoi(req.coap.bt_node);
	coap_send_request(&peer_addr, type, NULL, NULL, src, dst);
	NET_DBG("Send CoAP request '%s'-'%s' '%d - %d'", req.coap.command,
		req.coap.ipv6_addr, src, dst);
}
		    ...
		    u32_t mgmt_event, struct net_if *iface)
{
	...
	if (mgmt_event == NET_EVENT_IPV6_NBR_ADD) {
		...
		coap_send_request(&route_info->nexthop,
				  COAP_REQ_RPL_OBS, coap_obs_cb, NULL, 0, 0);
