...
#define RESPONSE_TIME  K_SECONDS(18)
...
static struct network_topology topology;

static const char * const uri_path[] = { "led", "0",  NULL };
static const char * const rpl_info_path[] = { "rpl-info", NULL };
...
static bool set_led_on(const struct sockaddr_in6 *peer, u16_t id)
{
	struct net_pkt *pkt;
	struct net_buf *frag;
	...
	if (r < 0) {
		NET_ERR("Failed to append payload marker");
		goto end;
	}

	r = coap_packet_append_payload(&request, (u8_t *)LED_ON,
				       strlen(LED_ON));
	if (r < 0) {
		NET_ERR("Failed to append payload marker");
		goto end;
...
static bool set_led_off(const struct sockaddr_in6 *peer, u16_t id)
{
	struct net_pkt *pkt;
	struct net_buf *frag;
	struct coap_packet request;
	...
	r = coap_packet_append_payload(&request, (u8_t *)LED_OFF,
				       strlen(LED_OFF));
	if (r < 0) {
		NET_ERR("Failed to append payload marker");
		goto end;
...
static bool get_rpl_info(const struct sockaddr_in6 *peer, u16_t id)
{
	struct net_pkt *pkt;
	struct net_buf *frag;
	struct coap_packet request;
...
static void request_timeout(struct k_work *work)
{
	...
	case COAP_REQ_NONE:
		return;
	case COAP_REQ_LED_ON:
		set_led_on(&request->peer, request->id);
		break;
	case COAP_REQ_LED_OFF:
		set_led_off(&request->peer, request->id);
		break;
	case COAP_REQ_RPL_INFO:
		get_rpl_info(&request->peer, request->id);
...
int coap_append_topology_info(struct net_pkt *pkt)
{
	...
	calculate_edges();
        ...
        pos = snprintk(buf, MAX_BUF_LEN, "{\"topology\": {");
	ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
	if (!ret) {
		return -ENOMEM;
	}

	pos = snprintk(buf, MAX_BUF_LEN, "\"nodes\": [");
	ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
	if (!ret) {
		return -ENOMEM;
	}
	...
	for (i = 0; i < CONFIG_NET_IPV6_MAX_NEIGHBORS; i++) {
		...
		pos = snprintk(buf, MAX_BUF_LEN, "%s", i == 0 ? "{" : ",{");
		...
		pos = snprintk(buf, MAX_BUF_LEN, "\"id\": %d, ",
			       topology.nodes[i].id);
		ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
		if (!ret) {
			return -ENOMEM;
		...
		pos = snprintk(buf, MAX_BUF_LEN, "\"label\": \"%s\", ",
			       topology.nodes[i].label);
		ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
		if (!ret) {
			return -ENOMEM;
		}

		pos = snprintk(buf, MAX_BUF_LEN, "\"title\": \"%s\"",
	...
	pos = snprintk(buf, MAX_BUF_LEN, "\"edges\": [");
	ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
	if (!ret) {
		return -ENOMEM;
	}
	...
	for (i = 0; i < CONFIG_NET_IPV6_MAX_NEIGHBORS; i++) {
		...
		pos = snprintk(buf, MAX_BUF_LEN, "\"from\": %d, ",
			       topology.edges[i].from);
		ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
		if (!ret) {
			return -ENOMEM;
		}

		pos = snprintk(buf, MAX_BUF_LEN, "\"to\": %d",
			       topology.edges[i].to);
		ret = net_pkt_append_all(pkt, pos, buf, PKT_WAIT_TIME);
		if (!ret) {
			return -ENOMEM;
		       ...
		       void *user_data)
{
	...
	if (!request) {
		...
		case COAP_REQ_NONE:
			return;
		case COAP_REQ_LED_ON:
			set_led_on(&peer, coap_next_id());
			break;
		case COAP_REQ_LED_OFF:
			set_led_off(&peer, coap_next_id());
			break;
		case COAP_REQ_RPL_INFO:
			get_rpl_info(&peer, coap_next_id());
		case COAP_REQ_RPL_OBS:
